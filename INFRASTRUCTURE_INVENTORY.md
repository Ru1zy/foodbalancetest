...
| Google Sheets service account | Writes global CRM, kitchen and monthly order rows | Current Google owner first | `GOOGLE_CLIENT_EMAIL`, `GOOGLE_PRIVATE_KEY` | `FoodBalance / Google service account` |
| Global CRM Sheet | `Info` client rows and `Orders` delivery-day rows | Current Google owner first | ID is stored only as `GOOGLE_SHEET_ID`; never use for kitchen export | `FoodBalance / Google Sheets destinations` |
| Kitchen Sheet | Separate manually-maintained kitchen/delivery export destination | Current Google owner first | ID is stored only as `EXTERNAL_SHEET_ID`; only the authenticated admin export action writes here | `FoodBalance / Google Sheets destinations` |
